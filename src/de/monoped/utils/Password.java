...
{
    static String chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabscdefghijklmnopqrstuvwxyz0123456789./";
	...
	public static String crypt(String strpw, String strsalt)
	{
		...
		byte[]  newEtr = new byte[etr.length];
		int     n = 0;
		int     m = 0;
		...
		System.arraycopy(etr, 0, newEtr, 0 , newEtr.length);
		ep = newEtr;
    ...
    /** Checks if a user passwd matches an encrypted passwd.
     *
     *  @param userPwd      Password given by user
     *  @param cryptedPwd   Encrypted password.
     *  @return             Tree if password matches.
     */

    public static boolean check(String userPwd, String cryptedPwd)
    {
        return crypt(userPwd, cryptedPwd.substring(0, 2)).equals(cryptedPwd);
    }
    ...
    public static String crypt(String strpw)
    {
        ...
        int     s1 = rand.nextInt(64),
                s2 = rand.nextInt(64);

        return crypt(strpw, "" + chars.charAt(s1) + chars.charAt(s2));
    }
